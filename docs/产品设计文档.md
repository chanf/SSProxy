...
- **实时连接**：5s 轮询活跃 TCP/UDP（上下行、出站策略、命中规则），支持模糊搜索（IP/设备名/策略/域名/规则）。
- **历史访问**：后台每 15s 增量去重持久化到 `/tmp/mihomo_access.log`。
- **全环境兼容**：用 `awk` 实现逆序/编号，规避低配固件裁剪 `tac`/`nl` 导致的空数据。
- **快捷加规则**：在连接旁一键将域名加入「代理 / 直连 / 拦截」。
...
| `docs/relay-chain-design.md` | 链式代理（Relay 多跳）+ 设备链路绑定设计 |
