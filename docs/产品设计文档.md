...
> 版本：1.0.0-199　·　最后更新：2026-07-22　·　状态：与代码同步
>
...
- **多订阅并用**：设置页维护多个订阅条目（名称 / 地址 / 启用），支持增删、启停和排序；排序具有语义，第一份启用订阅作为规则与策略组基准。
- **批量更新与合并**：`update_subscriptions` 同时更新所有启用订阅，独立缓存后合并节点池；后续订阅只贡献节点，不覆盖第一份订阅的规则与策略组。
- **节点去重**：按节点名去重，靠前订阅优先，避免同名节点重复导致核心配置异常。
- **聚合策略组**：多订阅时自动生成 `SSProxy - 全部订阅` select 组，并注入到第一份订阅的 select 策略组中；兼容块式列表、行内列表和 flow-map 策略组写法。
- **订阅自动更新**：自包含守护循环（每 10 分钟轮询、按小时间隔节流），不依赖系统 cron。
- **失败回退**：单份订阅更新失败时优先使用该订阅旧缓存；只有所有启用订阅都无可用缓存时才整体失败。
- **旧订阅链接兼容**：历史 `subscription_url` 自动迁移为第一条订阅，并继续保存到包外 `/etc/mihomo/.subscription_url` 作为升级兼容。
- **配置模式切换**：订阅 / 仅自定义 / 混合（自定义规则追加到订阅规则之后）。
- **配置合并** `prepare_config`：拷贝合并后的订阅 YAML 到 `/tmp/mihomo_run.yaml`，删原 `dns:`/`tun:`/端口，前置受控端口、追加受控 DNS/TUN 块，再注入 UCI 规则——UCI 的端口/DNS/TUN/规则设置才真正生效。
...
- **组合过滤**：设备/IP与出站策略可独立或组合筛选，自动刷新时保留筛选条件。
- **系统日志级别过滤**：仪表盘系统代理日志支持 Debug、Info、Warning、Error、Fatal 和其他级别。
...
1. **核心**：`restore_subscription_url` → `prepare_config` 基于已合并配置生成 `/tmp/mihomo_run.yaml` → 以 `-f /tmp/mihomo_run.yaml` 启动核心 → controller 就绪后由 `apply_network` 安装 TProxy/DNS 接管。
...
| `/etc/mihomo/config.yaml` | 多订阅合并后的源配置（`prepare_config` 的输入） |
| `/etc/mihomo/subscriptions/*.yaml` | 每个订阅的独立缓存，按 UCI section ID 命名 |
| `/etc/mihomo/.subscription_url` | 历史单订阅链接持久化（包外，升级兼容） |
...
### 6.3 多订阅合并与运行配置（`update_subscriptions` / `prepare_config`）

`update_subscriptions` 负责把多个远端订阅落盘到 `/etc/mihomo/subscriptions/<sid>.yaml`，再合并为 `/etc/mihomo/config.yaml`。第一份启用订阅是基准：保留它的 `proxy-groups:`、`rules:`、provider 与其它配置；后续订阅只抽取 `proxies:` 下按节点名去重后的新节点。多订阅时额外生成 `SSProxy - 全部订阅` 聚合 select 组，并把它加入基准订阅已有的 select 组，确保原规则出口能看见后续订阅节点。

`prepare_config` 再把合并后的源配置加工成 `/tmp/mihomo_run.yaml`：拷贝配置 → awk 删原 `dns:`/`tun:` 块 → sed 删顶层端口/controller → 前置受控端口 → 追加受控 `dns:`/`tun:` 块 → `emit_builtin_bypass_rules`（组播/LLMNR → DIRECT）+ `emit_access_rules_yaml`（UCI 规则）注入 `rules:` 顶部。
...
- **持久化**：策略组选择、多订阅配置和旧版订阅链接均跨重启/升级保留。
...
| `settings.js` | UCI 表单：多订阅列表/自动更新/更新间隔/延时 URL/TUN/DNS 劫持/IP 转发控制(含白名单)/高级端口路径/密钥/Geo |
...
- 访问日志可视化（流量趋势图）。
- 多订阅健康报表：展示每个订阅最近一次更新时间、缓存命中和失败原因。
