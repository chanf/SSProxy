...
### 已知限制（已修复/改进部分）
1. **组名/节点名已实现 URL 编码**（v1.0.0-86 已修复）：对 `select_node` 的组名/节点名实施了 `urlencode`，解除了特殊字符导致切换失败的限制，完美支持含中文、空格和 emoji 的策略组/节点。
2. **JSON 体中双引号已转义**（v1.0.0-86 已修复）：对传入的节点名双引号进行了自动转义处理，防止 JSON 载荷解析崩溃。
3. **已支持 secret 鉴权与动态端口**（v1.0.0-86 已实现）：后台通过 `get_api_config()` 自动拉取最新的密钥与端口，并通过 `mihomo_curl` 自带 `Authorization: Bearer <secret>` 报头发送请求，消除了原本鉴权失败的限制。
4. **不持久化**：核心重启回退到订阅默认节点（见 §6.3）。
5. **仅 Selector 可切**：自动类组不可手动指定（核心限制）。
6. **无组内延时展示**：切换面板不显示各候选节点延时（延时测试在「节点列表」区，按节点而非按组）。
