...
- **链式代理**：独立管理 SOCKS5 / HTTP / SS / Trojan / VMess / VLESS 落地节点，以「设备 IP → 订阅节点 → 落地节点」生成两跳链路；可按行选择前置节点，也可统一覆盖全部链路。
- **访问日志**：单表展示设备、访问目标、出站策略和上下行流量，保留最近 2000 条并支持一键清空。
...
后端 `/usr/share/mihomo/helper.sh` 为单体工具，`case` 分发约 30 个子命令（架构 / 订阅 / 配置合并 / 实时控制 / 规则 / 流量 / Geo / 链式代理）。前端 7 个纯 JS 视图，包括运行状态、服务设置、链式代理、访问日志、规则管理、流量统计和广告过滤。
...
├── Docs/               # 设计文档与测试用例
│   ├── 产品设计文档.md               # 整体产品设计（权威）
│   ├── chain-proxy-product-design.md # 链式代理产品设计
...
覆盖构建器（版本自增、tar 打包、可复现性、端到端）与 `helper.sh` 子命令黑盒（`prepare_config`、`get_proxies`、访问规则、链式代理、`emit_tproxy_rules`、LAN IP 探测等）。
...
| --- | --- |
| 🧭 产品经理 | chanf@me.com |
| 💻 研发 | Claude Code · Codex · OpenCode · GLM5.2 · GPT5.5 |
